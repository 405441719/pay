function luhmCheck(bankno){
    ...
    var arrJiShu=new Array(),arrJiShu2=new Array(),arrOuShu=new Array();
    for(var j=0;j<newArr.length;j++){
        if((j+1)%2==1){
            if(parseInt(newArr[j])*2<9) arrJiShu.push(parseInt(newArr[j])*2);else arrJiShu2.push(parseInt(newArr[j])*2);
        }else arrOuShu.push(newArr[j]);
    }
    var jishu_child1=new Array(),jishu_child2=new Array();
    for(var h=0;h<arrJiShu2.length;h++){
        jishu_child1.push(parseInt(arrJiShu2[h])%10);
        jishu_child2.push(parseInt(arrJiShu2[h])/10);
    }
    var sumJiShu=0,sumOuShu=0,sumJiShuChild1=0,sumJiShuChild2=0,sumTotal=0; //奇数位*2 < 9 的数组之和
    ...
    if(lastNum==luhm)return true; else return false; 
}
